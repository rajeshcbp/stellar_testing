...
<html lang="en">
<head>
    <title>Code coverage report for src/federation_server.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">all files</a> / <a href="index.html">src/</a> federation_server.js
    </h1>
    <div class='clearfix'>
      ...
      <div class='fl pad1y'>
        <span class="strong">21 statements, 1 function, 18 branches</span>
...
&nbsp;
   ...
   * @returns {Promise}
   */
  static resolve(value, opts<span class="branch-1 cbranch-no" title="branch not covered" > </span>= {}<span class="branch-1 cbranch-no" title="branch not covered" >)</span> {
...
&nbsp;
  ...
  resolveAddress(address) {
    if (address.indexOf('*') &lt; 0) {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.domain) {
<span class="cstat-no" title="statement not covered" >        return Promise.reject(new Error('Unknown domain. Make sure `address` contains a domain (ex. `bob*stellar.org`) or pass `domain` parameter when instantiating the server object.'));</span>
      }
...
&nbsp;
  _sendRequest(url) {
    return axios.get(url.toString(), {maxContentLength: FEDERATION_RESPONSE_MAX_SIZE})
      ...
      .catch(response =&gt; {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (response instanceof Error) {
...
<span class="cstat-no" title="statement not covered" >          return Promise.reject(new BadResponseError(`Server query failed. Server responded: ${response.status} ${response.statusText}`, response.data));</span>
        }
      });
  }
}
