...
<html lang="en">
<head>
    <title>Code coverage report for src/call_builder.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../index.html">all files</a> / <a href="index.html">src/</a> call_builder.js
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">90.4% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>113/125</span>
      </div>
      <div class='fl pad1y space-right2'>
      ...
      <div class='fl pad1y'>
        <span class="strong">3 statements, 5 branches</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import {NotFoundError, NetworkError, BadRequestError} from "./errors";
...
var EventSource = (typeof window === 'undefined') ? require('eventsource') : <span class="branch-1 cbranch-no" title="branch not covered" >window.EventSource;</span>
...
export class CallBuilder {
  constructor(serverUrl) {
    this.url = serverUrl;
    this.filter = [];
    this.originalSegments = this.url.segment() || <span class="branch-1 cbranch-no" title="branch not covered" >[];</span>
  }
&nbsp;
  /**
   * @private
...
&nbsp;
    var createTimeout = () =&gt; {
      timeout = setTimeout(<span class="fstat-no" title="function not covered" >() =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        es.close();</span>
<span class="cstat-no" title="statement not covered" >        es = createEventSource();</span>
...
<span class="cstat-no" title="statement not covered" >        if (options.onerror) {</span>
<span class="cstat-no" title="statement not covered" >          options.onerror(err);</span>
<span class="cstat-no" title="statement not covered" >          options.onerror('EventSource not supported');</span>
        }
...
&nbsp;
      es.onmessage = message =&gt; {
        var result = message.data ? this._parseRecord(JSON.parse(message.data)) : <span class="branch-1 cbranch-no" title="branch not covered" >message;</span>
        <span class="missing-if-branch" title="else path not taken" >E</span>if (result.paging_token) {
...
&nbsp;
      es.onerror = <span class="fstat-no" title="function not covered" >error =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        if (options.onerror) {</span>
<span class="cstat-no" title="statement not covered" >          options.onerror(error);</span>
        }
      };
&nbsp;
      return es;
...
&nbsp;
  ...
  _requestFnForLink(link) {
    return opts =&gt; {
      let uri;
&nbsp;
      <span class="missing-if-branch" title="else path not taken" >E</span>if (link.templated) {
        let template = URITemplate(link.href);
        uri = URI(template.expand(opts || {}));
      } else {
<span class="cstat-no" title="statement not covered" >        uri = URI(link.href);</span>
      }
&nbsp;
...
&nbsp;
  ...
  _toCollectionPage(json) {
    ...
    return {
      ...
      prev: <span class="fstat-no" title="function not covered" >() =&gt; {</span>
<span class="cstat-no" title="statement not covered" >        return this._sendNormalRequest(URI(json._links.prev.href))</span>
          .then(<span class="fstat-no" title="function not covered" >r </span>=&gt; this._toCollectionPage(r));
      }
    };
  }
&nbsp;
  ...
  _handleNetworkError(response) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (response instanceof Error) {
<span class="cstat-no" title="statement not covered" >      return Promise.reject(response);</span>
    } else {
      switch (response.status) {
        case 404:
          return Promise.reject(new NotFoundError(response.data, response));<span class="branch-1 cbranch-no" title="branch not covered" ></span>
        default:
<span class="cstat-no" title="statement not covered" >          return Promise.reject(new NetworkError(response.status, response));</span>
      }
    }
  }
&nbsp;
